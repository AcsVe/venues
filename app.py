...
-----END PRIVATE KEY-----""")
    ...
    if os.environ.get('DISABLE_REMINDERS') != '1':
        try:
            ...
            scheduler.add_job(
                func=lambda: check_and_send_daily_staff_summary(app),
                trigger='interval',
                minutes=10,
            ...
            # runs often so it actually fires close to the configured lead time.
            scheduler.add_job(
                func=lambda: check_and_send_upcoming_period_reminders(app),
                trigger='interval',
                minutes=5,
