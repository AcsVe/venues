    ...
    <div class="tab-panel" id="tab-contacts">
      <div class="card">
        ...
        <label style="display:flex;align-items:center;gap:8px;margin-bottom:14px;cursor:pointer;font-weight:600;font-size:.88rem">
          <input type="checkbox" id="ctNotifyHandover" style="width:17px;height:17px">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr>
              ...
              <th>المرحلة</th><th>إشعار تسليم الأجهزة</th><th>تاريخ الإضافة</th><th>حذف</th>
            </tr></thead>
...
<div class="tab-panel" id="tab-reports">
      ...
      <div class="card">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>رقم الحجز</th><th>المعلم/ة</th><th>التاريخ</th><th>المرحلة</th><th>الصف</th><th>الشعبة</th><th>الطالب</th><th>رقم الجهاز</th></tr></thead>
...
function showTab(name) {
  ...
  const titles = {bookings:t('طلبات الحجز'), halls:t('المراحل والعربات'), blocked:t('الفترات غير المتاحة'), contacts:t('جهات الاتصال'), teachers:t('المعلمون'), students:t('الطلاب'), reports:t('التقارير'), archive:t('الأرشفة'), calview:t('التقويم المرئي')};
...
function printLetterheadHtml(bodyHtml, titleAr, titleEn) {
  ...
  const logoImg = ORG_LOGO_URL ? `<img src="${ORG_LOGO_URL}" alt="logo" style="height:46px;display:block">` : '';

  return `<!DOCTYPE html><html ${dirAttr}><head>
<meta charset="UTF-8"><title>${title}</title>
<style>
  body{font-family:Arial,sans-serif;padding:24px;color:#1e2a2b;margin:0}
  .print-header{display:flex;align-items:center;gap:14px;border-bottom:3px solid #FF5733;padding-bottom:14px;margin-bottom:18px}
...
async function requestNotifyPermission() {
  ...
  if (perm === 'granted') {
    showToast(AR ? 'تم تفعيل الإشعارات ✓' : 'Notifications enabled ✓');
...
function openDetail(reqId) {
  ...
  document.getElementById('bkModalBody').innerHTML = `
    <table style="width:100%;border-collapse:collapse;font-size:.88rem">
      ...
      ${drow('الجوال', b.phone||'-')} ${drow('سبب الحجز', b.title||'-')}
...
function printBooking(reqId) {
  ...
  const html = printLetterheadHtml(body, `${AR?'تفاصيل الحجز':'Booking Details'} — ${b.reqId}`, `${AR?'تفاصيل الحجز':'Booking Details'} — ${b.reqId}`);
...
function renderContactsTable() {
  ...
  document.getElementById('contactsTable').innerHTML = list.map(c => `
    <tr>
      ...
      <td style="text-align:center"><input type="checkbox" ${c.notifyHandover?'checked':''} onchange="toggleContactNotifyHandover(${c.id}, this.checked)"></td>
      <td>${c.date}</td>
...
async function addContact() {
  ...
  const notifyHandover = document.getElementById('ctNotifyHandover').checked;
  if (!email) { showToast('البريد مطلوب', '#c0392b'); return; }
  const data = await api('/admin/api/add-contacts', {list: [{email, name, stageId, notifyHandover}]});
  if (data.success) {
    showToast(`تمت الإضافة (${data.count})`);
    document.getElementById('ctEmail').value = '';
    document.getElementById('ctName').value = '';
    document.getElementById('ctNotifyHandover').checked = false;
    loadContacts();
  } else showToast(data.error || 'خطأ', '#c0392b');
}

async function toggleContactNotifyHandover(id, checked) {
  const data = await api('/admin/api/update-contact', {id, notifyHandover: checked});
  if (data.success) {
    const c = lastContactsList.find(x => x.id === id);
    if (c) c.notifyHandover = checked;
    showToast(checked ? (AR?'تم تفعيل الإشعار':'Notification enabled') : (AR?'تم إيقاف الإشعار':'Notification disabled'));
  } else {
    showToast(data.error || t('خطأ'), '#c0392b');
    renderContactsTable(); // revert the visual checkbox state on failure
  }
}
...
function renderCheckoutLines() {
  ...
  document.getElementById('checkoutLinesTable').innerHTML = lines.map(l => `
    <tr>
      <td><strong style="color:var(--primary)">${esc(l.reqId)}</strong></td>
      <td>${esc(l.teacher)}</td>
      <td>${l.date}</td>
      <td>${esc(stageDisplayName(l.stage)||'')}</td>
      <td>${esc(gradeDisplayName(l.grade)||'')}</td>
      <td>${esc(l.section||'')}</td>
      <td>${esc(l.studentName||'')}</td>
      <td>${l.laptopNumber ?? `<span style="color:var(--muted)">${t('لم يُسلَّم')}</span>`}</td>
    </tr>`).join('') || `<tr><td colspan="8" style="text-align:center;color:var(--muted)">${t('لا توجد بيانات')}</td></tr>`;
}
  ...
  const tableHtml = `
    <table>
      <thead><tr>
        ...
        <th>${t('الطالب')}</th><th>${t('رقم الجهاز')}</th>
      </tr></thead>
...
async function sendCheckoutReportEmail() {
  ...
  // Send exactly what's currently filtered/visible, so the recipient sees the same view the admin is looking at.
  const visibleRows = [...document.querySelectorAll('#checkoutLinesTable tr')].map(tr => {
    const tds = [...tr.querySelectorAll('td')];
    if (tds.length < 8) return null;
    return {
      reqId: tds[0].textContent.trim(), teacher: tds[1].textContent.trim(), date: tds[2].textContent.trim(),
      stage: tds[3].textContent.trim(), grade: tds[4].textContent.trim(), section: tds[5].textContent.trim(),
      studentName: tds[6].textContent.trim(), laptopNumber: tds[7].textContent.trim(),
    };
  }).filter(Boolean);
...
const validTabs = ['bookings','halls','blocked','contacts','teachers','students','reports','archive','calview'];
